...
| Case diagrams | **All 26 cases carry a `flowchart` and a `sequenceDiagram`**, ASCII sketches removed. `06-ml-cases/feed-ranking.md` was the last gap and closed 2026-09-23 |
| Case **content** | **0 of 26 rewritten.** Every case still runs the old `## 1. Clarify … ## 8. Ops & cost` skeleton and carries none of the ten contract headings. Their `## Sources` heading is normalised; their frontmatter still says `difficulty:` where the contract says `tier:` |
